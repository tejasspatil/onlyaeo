...
const nextConfig = {
  ...
  // HTTP → HTTPS is enforced by Cloudflare at the edge ("Always Use HTTPS").
  // A worker-level redirect causes an infinite loop because Cloudflare terminates
  // TLS before the request reaches the worker, so x-forwarded-proto is always http.
